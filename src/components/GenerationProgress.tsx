...
interface GenerationProgressProps {
  status: "training" | "generating" | "completed" | string;
  description?: string;
  progress?: number;
}

export const GenerationProgress = ({ status, description, progress = 50 }: GenerationProgressProps) => {
  ...
  const title = statusTexts[status] || status;
  const desc = description || (status === "completed" 
